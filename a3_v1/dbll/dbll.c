#include <stdlib.h>
#include "dbll.h"

/* Routines to create and manipulate a doubly-linked list */

/* create a doubly-linked list */
/* returns an empty list or NULL if memory allocation failed */
struct dbll *dbll_create()
{
	// struct llnode *p;
	// p = (struct llnode *)malloc(sizeof(struct llnode));
	// p->next = NULL;
	// p->prev = NULL;
	struct dbll *L;
	L = (struct dbll *)malloc(sizeof(struct dbll));
	// if(L == NULL)
	// 	return NULL;
	L->first = NULL;
	L->last = NULL;
	
    return L;
}

/* frees all memory associated with a doubly-linked list */
/* this must also free all memory associated with the linked list nodes */
/* assumes user data has already been freed */
void dbll_free(struct dbll *list)
{
	struct llnode *p = list->first;
	struct llnode *q = p->next;
	while (p != NULL)
	{
		q = p->next;
		free(p);
		p = q;
	}
	// free(p);
	free(list);
}

/* iterate over items stored in a doubly-linked list */

/* begin iteration from node start, end iteration at node stop (include stop in iteration) */
/* the list is traversed using the next pointer of each node */

/* if start is NULL, then start is the first node */
/* if end is NULL, then end is the last node */

/* at each node, call the user function f with a pointer to the list,
   a pointer to the current node, and the value of ctx */

/* if f returns 0, stop iteration and return 1 */

/* return 0 if you reached the end of the list without encountering end */
/* return 1 on successful iteration */

int dbll_iterate(struct dbll *list,
				 struct llnode *start,
				 struct llnode *end,
				 void *ctx,
				 int (*f)(struct dbll *, struct llnode *, void *))
{
	if(start == NULL)
		start = list->first;
	if(end == NULL)
		end = list->last;
	struct llnode *p = start;
	while((*f)(list, p, ctx))
	{
		if(p != end && p != list->last)
			p = p->next;
		else if (p == end)
		{
			// (*f)(list, p, ctx);
			return 1;	
		}
		else
			return 0 ;
	}
	
  	return 1;
}

/* similar to dbll_iterate, except that the list is traversed using
   the prev pointer of each node (i.e. in the reverse direction).

   Adjust accordingly. For example if start is NULL, then start is the
   last node in the list.  If end is NULL, then end is the first node
   in the list.

*/

int dbll_iterate_reverse(struct dbll *list,
						 struct llnode *start,
						 struct llnode *end,
						 void *ctx,
						 int (*f)(struct dbll *, struct llnode *, void *)
						 )
{
	if(start == NULL)
		start = list->last;
	if(end == NULL)
		end = list->first;
	struct llnode *p = start;
	while((*f)(list, p, ctx))
	{
		if(p != end && p != list->first)
			p = p->prev;
		else if (p == end)
		{
			// (*f)(list, p, ctx);
			return 1;	
		}
		else
			return 0 ;
	}
  	return 1;
}


/* Remove `llnode` from `list` */
/* Memory associated with `node` must be freed */
/* You can assume user_data will be freed by somebody else (or has already been freed) */
void dbll_remove(struct dbll *list, struct llnode *node)
{
	// 1. 如果链表为空
	if (list->first == NULL)
		return;
	// 2. 链表仅有一个节点
	if(list->first == list->last)
	{
		list->first = NULL;
		list->last = NULL;
		free(node);
		return;
	}
	// 3. 链表有多个节点——待删除结点为first
	if(node == list->first)
	{
		list->first = node->next;
		list->first->prev = NULL;
		free(node);
		return;
	}
	// 4. 链表有多个节点——待删除结点为last
	if(node == list->last)
	{
		list->last = node->prev;
		list->last->next = NULL;
		free(node);
		return;
	}
	// 5. 一般情况
	struct llnode *p = node->prev;
	p->next = node->next;
	node->next->prev = p;
	free(node);
}

/* Create and return a new node containing `user_data` */
/* The new node must be inserted after `node` */
/* if node is NULL, then insert the node at the end of the list */
/* return NULL if memory could not be allocated */
struct llnode *dbll_insert_after(struct dbll *list, struct llnode *node, void *user_data)
{
    struct llnode *p;
	p = (struct llnode *)malloc(sizeof(struct llnode));
	// 1. return NULL if memory could not be allocated
	if(p == NULL)
		return NULL;
	
	p->user_data = user_data;


	//2. if the list is NULL, then this is the first node.
	if(list->first == NULL)
	{
		p->prev = NULL;
		p->next = NULL;
		list->first = p;
		list->last = p;
		return p;
	}

	// 3. if node == null, then insert the node at the end of the list
	if(node == NULL)
	{
		// 3.1 if the list is NULL, then this is the first node.
		// if(list->first == NULL)
		// {
		// 	p->prev = NULL;
		// 	p->next = NULL;
		// 	list->first = p;
		// 	list->last = p;
		// 	return p;
		// }

		// 3.2 if the list is not NULL
		list->last->next = p;
		p->prev = list->last;
		list->last = p;
		p->next =NULL;
		return p;
	}

	struct llnode *q = node->next;
	// 4. node is the last node
	if(q == NULL)
	{
		node->next = p;
		p->prev = node;
		p->next = NULL;
		list->last = p;
		return p;
	}
	// 5. general situation
	p->next = q;
	p->prev = node;
	node->next = p;
	q->prev = p;
	return p;
}

/* Create and return a new node containing `user_data` */
/* The new node must be inserted before `node` */
/* if node is NULL, then insert the new node at the beginning of the list */
/* return NULL if memory could not be allocated */
struct llnode *dbll_insert_before(struct dbll *list, struct llnode *node, void *user_data)
{
	struct llnode *p;
	p = (struct llnode *)malloc(sizeof(struct llnode));
	// 1. return NULL if memory could not be allocated
	if(p == NULL)
		return NULL;
	
	p->user_data = user_data;
	//2. if the list is NULL, then this is the first node.
	if(list->first == NULL)
	{
		p->prev = NULL;
		p->next = NULL;
		list->first = p;
		list->last = p;
		return p;
	}

	// 3. if node == null, then insert the new node at the beginning of the list
	if(node == NULL)
	{
		list->first->prev = p;
		p->next = list->first;
		list->first = p;
		p->prev = NULL;
		return p;
	}
	
	struct llnode *q = node->prev;
	// 4. node is the first node
	if(q == NULL)
	{
		node->prev = p;
		p->next = node;
		p->prev = NULL;
		list->first = p;
		return p;
	}
	// 5. general situation
	p->next = node;
	p->prev = q;
	node->prev = p;
	q->next = p;
	return p;
}

/* create and return an `llnode` that stores `user_data` */
/* the `llnode` is added to the end of list */
/* return NULL if memory could not be allocated */
/* this function is a convenience function and can use the dbll_insert_after function */
struct llnode *dbll_append(struct dbll *list, void *user_data)
{
    return dbll_insert_after(list, list->last, user_data);
}


